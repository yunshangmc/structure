...
/*********
 * 20190304@JAR:【3】处修改建议
 * （1:行372）函数'cnItem'优化建议，
 * （2:行415）函数'cntit'优化建议，
 * （3:行424）字典'cnItems'排序及译文优化建议；
 * 此注释经正式修改编辑后可删除。
 */
...
var cnItems = {
    ...
    'Activated artifacts can be equipped at "Sliders" tab.': '激活的工艺品可以在“滑块”选项卡上装备。',
    ...
    'Build shield': '建筑护盾',
    'Clone shield': '克隆护盾',
    'Imprinting continues even if you are exploring a virtual map, but ascending from map with unfinished imprints would lose you all unfinished imprints on that map. ': '即使您正在探索虚拟地图，也会继续进行印记，但是从具有未完成的印记的地图上升将会丢失该地图上所有未完成的印记。',
    'Summoning shield (looks like 8 spikes extending from node): Node can\'t be attacked by clones or summons.': '召唤盾牌（看起来像从节点延伸的8个尖刺）：节点不能被克隆或召唤攻击。',
    'You can imprint every fully leveled and built node on a real map.': '您可以在真实地图上打印每个完全平整和构建的节点。',
    'Feats are various tasks for you to complete. Most of feats unlock blueprint skills ': '专长是你要完成的各种任务。大多数专长解锁蓝图技能',
    'Planned nodes then can be bought to be actually activated and used. Once bought, node can\'t be refunded, only stored for later use.': '然后可以购买计划节点以实际激活和使用。 购买后，节点无法退款，仅存储以供日后使用。',
    'To activate nodes, you need workers. Every slider assigned to mine is considered a worker.': '要激活节点，需要工人。 分配给矿山的每个滑块都被视为工人。',
    ...
    'Summons produce gold while fighting (Additive: cap)': '战斗时召唤产生黄金(附加:帽子)',
    'Summons produce fears while fighting (Additive: cap)': '战斗时召唤产生恐惧(附加:帽子)',
    'Sliders get random spirit charge boost whenever a point is captured': '当一个点被捕获时，滑块会获得随机的精神充能提升',
    'Each real slider gains total metal growth of every slider': '每个实际滑块都可以获得每个滑块的总金属增长',
    'Each real slider gains total blood growth of every slider': '每个实际滑块都可以获得每个滑块的总血液增长',
    ...
    'Elemental growth cap doubled': '元素增长上限翻倍',
    'Imprintable nodes start imprinting as soon as possible': '可打印节点尽快开始打印',
    'Power growth cap doubled': '能量增长上限翻了一番',
    'World core': '世界核心',
    'or core fragments': '或核心碎片',
    'For each world you can refine its core, which decreases number of memories you can use. Most core fragments have threshold building number requirements to activate. You can\'t activate three fragments that share a single corner.': '对于每个世界，您可以优化其核心，从而减少您可以使用的内存数量。 大多数核心片段都有激活的阈值建筑物数量要求。 您无法激活共享一个角落的三个片段。',
    'Mean machines only deal 10% damage to it.': '意味着机器只对它造成10%的伤害。',
    'Narrow path (looks like two borders at sides of path): Only one slider can attack the node at a time.': '窄路径（看起来像路径两侧的两个边框）：一次只有一个滑块可以攻击节点。',
    'You can use memories to build your own world and upgrade its core.': '您可以使用内存来建立自己的世界并升级其核心。',
    'Imprinting continues even if you are exploring another map, as long as the map node belongs to is still on the list. Ascending from map with unfinished imprints or deleting such virtual map would lose you all unfinished imprints on that map. ': '即使您正在探索另一个地图，只要地图节点所属的仍在列表中，就会继续进行打印。 从具有未完成的印记的地图升级或删除此类虚拟地图将丢失该地图上所有未完成的印记。',
    'Completed evolved virtual maps that have same level as a real map produce stardust over time.': '完成与真实地图具有相同级别的演化虚拟地图会随着时间的推移产生星尘。',
    'Anti-channel shield (looks like double hexagon around node): Channelling is cut off for sliders attacking such node.': '反通道屏蔽（看起来像节点周围的双六边形）：对于攻击此节点的滑块，切断通道。',
    'Master controls': '主控制',
    'Master controls allow you to override various individual settings. You can also save sliders setups as presets. If a slider was not present while preset was saved, it wont be affected by loading, except all artifacts will be unequipped. Presets only save settings, attribute values persist.': '主控制允许您覆盖各种个人设置。 您还可以将滑块设置保存为预设。 如果在保存预设时没有滑块，则不会受到加载的影响，除非所有工件都没有装备。 预设仅保存设置，属性值保持不变。',
    'Every element ignores itself, deals quadruple damage to the next, straight damage to opposite element, halved damage to previous one.': '每种元素都不能对同种元素节点造成伤害，对下一种元素造成四倍伤害，对对位的元素造成全额的伤害，对上一种元素造成一半的伤害',
    'Elements go in the same order they are usually listed : Blood beats fire, Fire beats ice, Ice beats metal, Metal beats blood.': '元素总是遵循这个规律：血克制火，火克制冰，冰克制金属，金属克制血',
    'Clone factory (looks like a cogwheel): A clone will join you as you capture the node. Clones ignore spirit penalty. They are created with mean power value of your sliders and can\'t grow. Clones can\'t ascend with you. Channelling upon clones increases their power.': '克隆工厂（齿轮形状）：你将在占领后获得一个克隆.克隆不会受到精神惩罚.克隆的初始力量取决于你所有滑块的力量平均值并且不能继续成长.克隆不能跟随你进入下一地图.所有通道都会增加克隆的力量.',
    'Clones receive any channeling as bonus to their power.': '克隆可以从任何通道中获得力量.',
    '': '',
    '': '',
    '': '',
    '': '',
