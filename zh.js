...
var cnItems = {
    ...
    'First, you plan out your node placement. There are some rules on how you can place nodes: ↵- Every node has a "dead zone". No node can intersect with it.↵- Every node has a "reach zone". Every pair of node with intersecting reach zones will be connected.↵- Nodes of same type (color) can\'t be connected.': '首先，计划节点放置。 关于如何放置节点有一些规则：↵- 每个节点都有一个“死区”。 没有节点可以与它相交.↵- 每个节点都有一个“到达区域”。 将连接具有相交到达区域的每对节点。↵- 不能连接相同类型（颜色）的节点。',
    ...
    'To activate nodes, you need workers. Every slider assigned to mine is considered a worker.': '要激活节点，需要工人。 分配给我的每个滑块都被视为工人。',
